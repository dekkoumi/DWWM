...
function calculJourAnniv(_mois,_jour){
   ...
   let dateAnniv=new Date(dateDuJour.getFullYear(),_mois-1,_jour);
   let jourAnniv=0;
   if (dateDuJour<dateAnniv) {
       jourAnniv=Math.ceil( (dateAnniv-dateDuJour)/(24*60*60*1000));
   }
   else{
       dateAnniv=new Date(dateDuJour.getFullYear+1,_mois-1,_jour);
       jourAnniv=Math.ceil( (dateAnniv-dateDuJour)/(24*60*60*1000));
   }
   return jourAnniv;
}
